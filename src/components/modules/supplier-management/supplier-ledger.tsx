...
export function SupplierLedger({ 
  ...
  useEffect(() => {
    loadData()
  }, [supplierId, toast, loadData])

  // Add real-time listener for purchases to automatically refresh when payments are made
  useEffect(() => {
    if (!supplierId) return

    console.log("Setting up real-time listener for purchases of supplier:", supplierId)
    
    const unsubscribe = PurchaseService.listenToPurchasesBySupplier(supplierId, (supplierPurchases) => {
  ...
  // Add real-time listener for supplier credits to automatically refresh when payments are made
  useEffect(() => {
    if (!supplierId) return

    console.log("Setting up real-time listener for supplier credits of supplier:", supplierId)
    
    const unsubscribe = SupplierCreditService.listenToCreditsBySupplier(supplierId, (supplierCredits) => {
    ...
    return () => {
      console.log("Cleaning up real-time listener for supplier credits")
      unsubscribe()
    }
  }, [supplierId, supplierName])

  // Add real-time listener for supplier credit transactions
  useEffect(() => {
    if (!supplierId) return

    console.log("Setting up real-time listener for supplier credit transactions of supplier:", supplierId)
    
    const unsubscribe = SupplierCreditService.listenToCreditTransactions(supplierId, (transactions) => {
    ...
    return () => {
      console.log("Cleaning up real-time listener for supplier credit transactions")
      unsubscribe()
    }
  }, [supplierId, supplierName])
  ...
  const ledgerTransactions: LedgerTransaction[] = useMemo(() => {
    ...
    purchases.forEach(purchase => {
      ...
      // Only add if there's no payment history or if the partial amount is different from payment history total
      ...
      if (partialAmount > 0 && partialAmount !== paymentHistoryTotal) {
        allTransactions.push({
          id: `partial-payment-${purchase.id}`,
          date: purchase.createdAt,
          type: "payment",
          amount: partialAmount,
          detail: `Partial Payment for ${purchase.invoiceNumber}`,
          invoiceNumber: purchase.invoiceNumber,
          description: `Partial payment of Rs${partialAmount.toLocaleString()} for invoice ${purchase.invoiceNumber}`,
          createdAt: purchase.createdAt,
          runningBalance: 0 // Will be calculated after sorting
        })
      }

...
interface EditableCartSalesProps {
  items: CartItem[]
  onItemsChange: (items: CartItem[]) => void
  onTotalChange: (subtotal: number, discount: number, total: number) => void
}

export function EditableCartSales({ items, onItemsChange, onTotalChange }: EditableCartSalesProps) {
    ...
    if (existingItem) {
      updatedItems = items.map((item) => {
        if (item.id === product.id) {
          ...
          const newIndividualPrices = [...(item.individualPrices || []), product.currentPrice]
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <p className="text-xs text-green-600 font-semibold">Rs{product.currentPrice.toLocaleString()}</p>
                        {isInCart && (
                  ...
                  <TableBody>
                    {items.map((item) => (
                      <TableRow key={item.id}>
                        ...
                        <TableCell className="text-foreground">
                          <div className="text-sm">
                            <p className={`font-medium ${item.availableStock < item.quantity ? 'text-red-500' : item.availableStock <= 5 ? 'text-orange-500' : 'text-green-600'}`}>
                              {item.availableStock}
                            </p>
                            {item.availableStock < item.quantity && (
                              <p className="text-xs text-red-500">Insufficient stock</p>
                            )}
                            {item.availableStock <= 5 && item.availableStock >= item.quantity && (

...
export function CustomerLedger({ 
  ...
  useEffect(() => {
    ...
    const unsubscribeSales = SalesService.listenToSalesByCustomer(customerName, customerPhone, (salesData) => {
      ...
      setIsUpdating(true)
      setSales(salesData)
      setTimeout(() => setIsUpdating(false), 1000) // Show updating indicator for 1 second
    })
  ...
  const ledgerTransactions: LedgerTransaction[] = useMemo(() => {
    ...
    if (sales && Array.isArray(sales)) {
      ...
      allTransactions.push({
        ...
        type: "sale",
        amount: saleAmount,
        detail: `Sale - ${sale.items.map(item => item.name).join(', ')}`,
    ...
    transactions.forEach(transaction => {
      if (transaction.type === "used") {
        allTransactions.push({
          id: `credit-used-${transaction.id}`,
          date: transaction.createdAt,
          type: "payment",
          amount: transaction.amount,
          detail: `Credit used for ${transaction.invoiceNumber}`,
          invoiceNumber: transaction.invoiceNumber || `CREDIT-${transaction.id}`,
    ...
    const transactionsWithBalance = chronologicalTransactions.map(transaction => {
      if (transaction.type === "sale" || transaction.type === "debit") {
        // Sale or debit increases balance (customer owes more)
  ...
  }, [sales, credits, transactions, initialBalance, loading])
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      const [salesData, creditsData, transactionsData] = await Promise.all([
        SalesService.getAllSales().then(sales => 
          sales.filter(sale => 
            sale.customerName === customerName && sale.customerPhone === customerPhone
          )
        ),
        CustomerCreditService.getCreditsByCustomer(customerId),
        CustomerCreditService.getCreditTransactions(customerId)

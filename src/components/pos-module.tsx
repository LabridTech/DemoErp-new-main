...
import {
  ...
  Percent, MapPin, Users, Zap, Loader2, X, CheckCircle2, Receipt
} from "lucide-react"
import {
...
function ProductCard({ product, quantity, onAdd, onRemove }: {
  ...
  return (
    <div className={`
      group relative flex flex-col rounded-2xl border transition-all duration-200 overflow-hidden bg-card
      ${outOfStock
      ...
      <div className="p-3 flex flex-col gap-2 flex-1">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-1 mt-1.5">
            ...
            {product.size && (
              <span className="text-[10px] text-muted-foreground bg-muted/60 rounded px-1.5 py-0.5">{product.size}</span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-end justify-between gap-1">
          <div>
            <p className="text-sm font-bold text-foreground">Rs {product.currentPrice.toLocaleString()}</p>
            <p className={`text-[10px] mt-0.5 ${outOfStock ? 'text-red-500 dark:text-red-400 font-medium'
                : stockLow ? 'text-amber-600 dark:text-amber-400 font-medium'
          ...
          {/* stepper */}
          {outOfStock ? (
            <div className="text-[10px] text-muted-foreground bg-muted/50 rounded-lg px-2 py-1">Unavailable</div>
          ) : (
            <div className="flex items-center gap-1.5">
              ...
              <span className={`w-6 text-center text-xs font-bold tabular-nums
                ${quantity > 0 ? 'text-blue-600 dark:text-blue-400' : 'text-muted-foreground'}`}>
                {quantity}
              </span>
              <button
                onClick={onAdd}
...
function CartRow({ item, product, onQtyChange, onQtyInput, onPriceChange, onFreeChange, onRemove, onAdvancedPrice, cartDiscountPercentage }: {
      ...
      <div className="flex items-start gap-2 mb-2.5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 flex-wrap">
            ...
            <span className="text-[10px] text-muted-foreground bg-muted rounded px-1 py-0.5">{item.code}</span>
            {stockIssue && <span className="text-[10px] font-medium text-red-700 dark:text-red-400 bg-red-100 dark:bg-red-950/50 rounded px-1 py-0.5 flex items-center gap-0.5"><AlertTriangle className="w-2.5 h-2.5" />Stock</span>}
            {item.tradeDiscountFreeItems! > 0 && <span className="text-[10px] font-medium text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 rounded px-1 py-0.5 flex items-center gap-0.5"><Gift className="w-2.5 h-2.5" />{item.tradeDiscountFreeItems} free</span>}
            {priceModified && <span className="text-[10px] font-medium text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 rounded px-1 py-0.5">modified</span>}
          </div>
          {product?.size && <p className="text-[10px] text-muted-foreground mt-0.5">Size {product.size}{stockIssue ? ` · available: ${product.stock} yds` : ''}</p>}
        </div>
        <button onClick={onRemove} className="w-6 h-6 rounded-lg flex items-center justify-center text-muted-foreground hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors flex-shrink-0">
      ...
      <div className="grid grid-cols-3 gap-2 ml-9">
        ...
        <div>
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => onQtyChange(-1)} className="w-6 h-6 rounded-md border border-border/60 flex items-center justify-center text-muted-foreground hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors">
            ...
            <Input
              ...
              className="h-6 w-12 text-center text-[11px] px-1 border-border/60 rounded-md [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield]"
            />
            <button onClick={() => onQtyChange(1)} disabled={product ? item.quantity >= product.stock : false} className="w-6 h-6 rounded-md border border-emerald-200 dark:border-emerald-800 flex items-center justify-center text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-600 hover:text-white disabled:opacity-30 transition-colors">
        ...
        <div>
          ...
          <div className="flex items-center gap-1">
            <Input
              ...
              onFocus={e => { if (e.target.value === "0") e.target.value = "" }}
              onBlur={e => { if (!e.target.value) e.target.value = "0" }}
              className="h-6 flex-1 text-[11px] px-1.5 border-border/60 rounded-md [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield]"
            />
            <button onClick={onAdvancedPrice} className="w-6 h-6 rounded-md border border-violet-200 dark:border-violet-800 flex items-center justify-center text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-950/40 hover:bg-violet-600 hover:text-white transition-colors" title="Advanced pricing">
        ...
        <div>
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => onFreeChange(Math.max(0, (item.tradeDiscountFreeItems || 0) - 1))} className="w-6 h-6 rounded-md border border-border/60 flex items-center justify-center text-muted-foreground hover:bg-red-50 hover:text-red-600 transition-colors">
              <Minus className="w-2.5 h-2.5" />
            </button>
            <Input
              type="number" step="0.01" min="0"
              ...
              onFocus={e => { if (e.target.value === "0") e.target.value = "" }}
              onBlur={e => { if (!e.target.value) e.target.value = "0" }}
              className="h-6 w-10 text-center text-[11px] px-1 border-border/60 rounded-md [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield]"
            />
            <button onClick={() => onFreeChange((item.tradeDiscountFreeItems || 0) + 1)} className="w-6 h-6 rounded-md border border-emerald-200 dark:border-emerald-800 flex items-center justify-center text-emerald-600 bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-600 hover:text-white transition-colors">
              <Plus className="w-2.5 h-2.5" />
            </button>
          </div>
        </div>
...
function POSModule() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setProducts(unique); setEmployees(e); setLoading(false)
      } catch {
  ...
  useEffect(() => {
    CustomerService.getAllCustomers().then(setCustomers).catch(console.error)
  }, [])
  ...
  const syncCart = (updated: CartItem[]) => { clearCart(); updated.forEach(i => contextAddToCart(i)) }

  const recalcDiscount = (updated: CartItem[]) => {
  ...
  const addToCart = (product: Product) => {
    ...
    if (existing) {
      if (existing.quantity + 1 > product.stock) {
        toast({ title: "Insufficient Stock", description: `Only ${product.stock} available`, variant: "destructive" }); return
      }
      updated = cart.map(i => i.id === product.id
        ? { ...i, quantity: i.quantity + 1, finalPrice: (i.quantity + 1) * i.unitPrice, individualPrices: [...(i.individualPrices || []), product.currentPrice], totalAmount: (i.totalAmount || 0) + product.currentPrice }
        : i)
    } else {
      if (product.stock <= 0) { toast({ title: "Out of Stock", variant: "destructive" }); return }
      updated = [...cart, { id: product.id, name: product.name, code: product.code, unitPrice: product.currentPrice, quantity: 1, discount: 0, finalPrice: product.currentPrice, availableStock: product.stock, fabricType: product.fabricType, size: product.size, individualPrices: [product.currentPrice], totalAmount: product.currentPrice }]
    }
  ...
  const updateQuantity = (id: string, newQty: number) => {
    ...
    if (newQty > product.stock) { toast({ title: "Insufficient Stock", description: `Only ${product.stock} available`, variant: "destructive" }); return }
    const updated = cart.map(i => i.id === id ? { ...i, quantity: newQty, finalPrice: newQty * i.unitPrice, totalAmount: newQty * i.unitPrice, individualPrices: [] } : i)
    syncCart(updated); recalcDiscount(updated)
  }
  ...
  const hasStockIssues = cart.some(item => { const p = products.find(p => p.id === item.id); return p ? item.quantity > p.stock : false })
  ...
  const handlePriceUpdate = (productId: string, newPrice: number, individualPrices: number[]) => {
    ...
    const updated = cart.map(i => i.id === productId ? { ...i, unitPrice: newPrice, finalPrice: t, individualPrices, totalAmount: t } : i)
    syncCart(updated); recalcDiscount(updated)
  }
  ...
  const matchPhone = (v: string, cp: string) => {
    const norm = (n: string) => n.replace(/[+\-\s]/g, '')
    const nv = norm(v), nc = norm(cp)
    if (nv.startsWith('92') && nc.startsWith('0')) { const a = nv.slice(2), b = nc.slice(1); return b === a || b.includes(a) || a.includes(b) }
    if (nv.startsWith('0') && nc.startsWith('92')) { const a = nv.slice(1), b = nc.slice(2); return b === a || b.includes(a) || a.includes(b) }
  ...
  const handleCustomerNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (v.length > 0) {
      const filtered = customers.filter(c => (c.name || '').toLowerCase().includes(v.toLowerCase()) || (c.address || '').toLowerCase().includes(v.toLowerCase()) || (v.match(/^[0-9+\-\s]/) && matchPhone(v, c.phone)))
      setCustomerSuggestions(filtered.slice(0, 8))
    } else setCustomerSuggestions([])
  }

  const selectCustomer = (c: Customer) => {
    setCustomerName(c.name || ''); setCustomerPhone(c.phone || ''); setCustomerAddress(c.address || ''); setCustomerSuggestions([])
  }
  ...
  const handleCheckout = async () => {
    if (!cart.length) { toast({ title: "Empty cart", description: "Add items before checkout", variant: "destructive" }); return }
    ...
    if (paymentMethod === "credit" && (!customerName.trim() || !customerPhone.trim())) {
      toast({ title: "Customer info required for credit sales", variant: "destructive" }); return
    }
    setIsProcessingSale(true)
    const stockIssues = cart.filter(item => { const p = products.find(p => p.id === item.id); return p ? item.quantity > p.stock : false })
    if (stockIssues.length) {
      toast({ title: "Stock issues", description: stockIssues.map(i => `${i.name}: need ${i.quantity}`).join(', '), variant: "destructive" })
      setIsProcessingSale(false); return
    }
    try {
      ...
      const saleItems: SaleItem[] = cart.map(item => {
        ...
        return { id: item.id, name: item.name, code: item.code, quantity: item.quantity, originalPrice: item.unitPrice, finalPrice: item.unitPrice - (dist[item.id] ?? 0) / (item.quantity || 1), discount: dist[item.id] ?? 0, purchaseCost: p?.purchaseCost || 0 }
      })
      ...
      const saleData: Omit<SaleRecord, "id"> = {
        invoiceNumber, date: now.toISOString().split('T')[0], time: now.toLocaleTimeString(),
        customerName: customerName || "Walk-in Customer", customerPhone: customerPhone || "", customerAddress: customerAddress || "",
        customerType: (customerName ? "regular" : "walk-in") as "walk-in" | "regular" | "vip",
        items: saleItems, subtotal, discount: totalDiscount, tax: 0, total,
        ...
        deliveryStatus: deliveryType === 'delivery' ? 'pending' : 'pickup', deliveryType,
        staffMember, staffName: staffNameForInvoice, notes: "", returnStatus: "none" as const,
        createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(),
      }
      if (deliveryType === 'delivery') {
        saleData.deliveryAddress = deliveryAddress || ""
        if (deliveryDate?.trim()) saleData.deliveryDate = deliveryDate
      }
      ...
      if (staffMember) {
        const emp = employees.find(e => e.id === staffMember)
        if (emp) await EmployeePerformanceService.incrementSalesMetrics(staffMember, emp.name, total).catch(console.error)
      }
      ...
      setShowPostSaleModal(true); resetForm()
    } catch {
      toast({ title: "Error", description: "Failed to complete sale. Try again.", variant: "destructive" })
  ...
  const handleThermalPrint = async () => {
    ...
    w.document.write(generateThermalInvoiceHTML(lastSaleData)); w.document.close()
      ...
      w.onload = () => setTimeout(async () => {
        try {
          ...
          if (navigator.share && navigator.canShare?.({ files: [new File([blob], 'invoice.png', { type: 'image/png' })] })) {
            await navigator.share({ title: `Invoice #${data.invoiceNumber}`, text: msg, files: [new File([blob], 'invoice.png', { type: 'image/png' })] })
            toast({ title: "Invoice shared!" }); w.close(); return
          }
          const url = URL.createObjectURL(blob)
          const a = document.createElement('a'); a.href = url; a.download = `Invoice_${data.invoiceNumber}.png`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url)
    ...
    <div className="flex items-center justify-center h-screen bg-background">
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="text-center">
          <p className="font-semibold text-foreground">Loading POS</p>
    ...
    <>
      <div className="flex h-screen overflow-hidden bg-muted/20 dark:bg-background">
        ...
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          {/* Search bar */}
          <div className="flex-shrink-0 bg-background border-b border-border/50 px-5 py-3 flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              <span className="bg-muted rounded-full px-2.5 py-1 font-medium">{filteredProducts.length} products</span>
              {hasStockIssues && (
                <span className="bg-red-50 dark:bg-red-950/40 text-red-600 dark:text-red-400 rounded-full px-2.5 py-1 font-medium flex items-center gap-1">
                  <AlertTriangle className="w-3 h-3" /> Stock issue
                </span>
              )}
            </div>
          </div>
            ...
            {filteredProducts.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full gap-3 text-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground/60 mt-0.5">Try a different search term</p>
                </div>
              </div>
            ) : (
        ...
        {/* ══ RIGHT: Order Panel ══════════════════════════════════════════════ */}
        <div className="w-[400px] flex-shrink-0 border-l border-border/50 bg-background flex flex-col overflow-hidden">
          ...
          <div className="flex-shrink-0 px-5 py-4 border-b border-border/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <p className="text-sm font-bold text-foreground leading-tight">Order</p>
                  <p className="text-[11px] text-muted-foreground">Bin Sultan Fabrics</p>
                </div>
              </div>
              {cart.length > 0 && (
                <span className="text-xs font-medium bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-400 rounded-full px-2.5 py-1">
          ...
          <div className="flex-1 overflow-y-auto">
                ...
                {cart.map((item, idx) => {
                  ...
                  return (
                    <CartRow
                      ...
                      item={item}
                      product={product}
                      onQtyChange={delta => updateQuantity(item.id, item.quantity + delta)}
                      onQtyInput={val => handleQtyInput(item.id, val)}
                      onPriceChange={price => updatePrice(item.id, price)}
                      onFreeChange={free => updateFree(item.id, free)}
            ...
            <div className="px-4 py-3">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Discount</span>
              </div>
              <div className="flex gap-2">
                <div className="flex-1">
                  <p className="text-[10px] text-muted-foreground mb-1">Amount (Rs)</p>
                  <Input
                    ...
                    className="h-8 text-sm rounded-xl [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none [-moz-appearance:textfield]"
                  />
                </div>
                <div className="flex-1">
            ...
            <div className="px-4 py-3 border-t border-border/40">
              <div className="flex items-center gap-1.5 mb-2.5">
                ...
                {paymentMethod === "credit" && <span className="text-[10px] text-red-500 font-medium">· required</span>}
              </div>
              <div className="space-y-2 relative">
                <div className="relative">
                  <Input
                    ...
                    autoComplete="off"
                    className={`h-8 text-sm rounded-xl pr-8 ${paymentMethod === "credit" && !customerName.trim() ? "border-red-400 focus:border-red-400" : ""}`}
                  />
                  {customerName && (
                    <button onClick={() => { setCustomerName(''); setCustomerSuggestions([]) }} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
                      <X className="w-3 h-3" />
                    </button>
                  )}
                </div>
                ...
                {customerSuggestions.length > 0 && (
                  <div className="absolute left-0 right-0 z-30 bg-background border border-border rounded-xl shadow-lg overflow-hidden">
                    <div className="px-3 py-1.5 bg-muted/40 border-b border-border/50 text-[10px] text-muted-foreground font-medium">
                      {customerSuggestions.length} match{customerSuggestions.length !== 1 ? 'es' : ''}
                    </div>
                    {customerSuggestions.map(c => (
                      <div key={c.id} onClick={() => selectCustomer(c)} className="flex items-center justify-between px-3 py-2.5 hover:bg-muted/50 cursor-pointer border-b border-border/30 last:border-0">
                        <div className="min-w-0">
                          ...
                          <p className="text-[11px] text-muted-foreground">{c.phone}{c.address ? ` · ${c.address}` : ''}</p>
                        </div>
                  ...
                  <Input
                    ...
                    onChange={e => {
                      ...
                      if (e.target.value.length > 3) {
                        const m = customers.filter(c => matchPhone(e.target.value, c.phone))
                        if (m.length <= 5) setCustomerSuggestions(m)
                      } else setCustomerSuggestions([])
                    }}
                    autoComplete="off"
                    className={`h-8 text-sm rounded-xl ${paymentMethod === "credit" && !customerPhone.trim() ? "border-red-400" : ""}`}
                  />
                  <Input
                    placeholder="Address"
                    ...
                    onChange={e => {
                      ...
                      if (e.target.value.length > 2) { const m = customers.filter(c => c.address?.toLowerCase().includes(e.target.value.toLowerCase())); if (m.length <= 5) setCustomerSuggestions(m) } else setCustomerSuggestions([])
                    }}
                    autoComplete="off"
            ...
            <div className="px-4 py-3 border-t border-border/40">
              <div className="flex items-center gap-1.5 mb-2.5">
                ...
                <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Delivery</span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                ...
                {deliveryType === 'delivery' && (
                  <Input type="date" value={deliveryDate} onChange={e => setDeliveryDate(e.target.value)} className="h-8 text-sm rounded-xl" />
                )}
              </div>
              {deliveryType === 'delivery' && (
                <Input placeholder="Delivery address" value={deliveryAddress} onChange={e => setDeliveryAddress(e.target.value)} className="h-8 text-sm rounded-xl mt-2" />
              )}
            </div>
            ...
            <div className="px-4 py-3 border-t border-border/40">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Select value={staffMember} onValueChange={setStaffMember}>
                  <SelectTrigger className="h-8 text-sm rounded-xl"><SelectValue placeholder="Select staff" /></SelectTrigger>
                  <SelectContent>
                    {employees.map(e => <SelectItem key={e.id} value={e.id}>{e.name}</SelectItem>)}
                  </SelectContent>
                </Select>
                <Input placeholder="Or type name" value={manualStaffName} onChange={e => setManualStaffName(e.target.value)} className="h-8 text-sm rounded-xl" />
              </div>
            </div>
            ...
            <div className="px-4 py-3 border-t border-border/40">
              <div className="flex items-center gap-1.5 mb-2.5">
                ...
                <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Payment</span>
              </div>
              ...
              {paymentMethod === "credit" && (
                <div className="mt-2.5 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/50 rounded-xl p-3 space-y-2">
                  ...
                  <Input
                    type="number" placeholder="Enter amount" min="0"
                    value={partialPaymentAmount}
                    onChange={e => setPartialPaymentAmount(e.target.value)}
                    className="h-8 text-sm rounded-xl bg-background"
                  />
                  {partialPaymentAmount && parseFloat(partialPaymentAmount) > 0 && (
                    <p className="text-[11px] text-amber-700 dark:text-amber-400">
                      Remaining: <strong>Rs {(total - parseFloat(partialPaymentAmount)).toLocaleString()}</strong>
                    </p>
                  )}
                </div>
              )}
          ...
          {/* ── Totals + Checkout (pinned to bottom) ── */}
          <div className="flex-shrink-0 border-t border-border/50 bg-background">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground flex items-center gap-1">
                    ...
                    {cartDiscountPercentage > 0 && (
                      <span className="text-[10px] bg-red-50 dark:bg-red-950/40 text-red-600 dark:text-red-400 rounded px-1 font-medium">{cartDiscountPercentage}%</span>
                    )}
                  </span>
              ...
              {paymentMethod === "credit" && partialPaymentAmount && parseFloat(partialPaymentAmount) > 0 && (
                <>
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium text-emerald-600">Rs {parseFloat(partialPaymentAmount).toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Remaining</span>
                    <span className="font-medium text-amber-600">Rs {(total - parseFloat(partialPaymentAmount)).toLocaleString()}</span>
                  </div>
                </>
              )}
              <div className="flex justify-between items-baseline pt-1 border-t border-border/40">
                <span className="text-sm font-bold text-foreground">Total</span>
                <span className="text-2xl font-bold text-foreground tabular-nums">Rs {total.toLocaleString()}</span>
              </div>
            </div>
            ...
            <div className="px-4 pb-4">
              <Button
                ...
                className="w-full h-11 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm disabled:bg-muted disabled:text-muted-foreground transition-all"
              >
                {isProcessingSale
                  ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Processing…</>
                  : <><Zap className="w-4 h-4 mr-2" />Complete Sale</>
                }
              </Button>
              {!paymentMethod && cart.length > 0 && (
                <p className="text-center text-[11px] text-muted-foreground mt-1.5">Select a payment method to continue</p>
              )}
            </div>
          </div>
        </div>
